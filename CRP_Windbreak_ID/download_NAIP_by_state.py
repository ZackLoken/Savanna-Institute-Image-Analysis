...
if len(sys.argv) < 4:
    print("Usage: python download_naip_by_state.py <state_names:str> <num_workers:int> <out_dir:str>")
    print('Example: python download_naip_by_state.py "Iowa,Illinois" 24 "/path/to/output"')
    sys.exit(1)
...
def process_sub_region(state_name, county_name, sub_region_idx, sub_region, failed_sub_regions, lock, start_time, request_counter):
    ...
    with lock:
        request_counter.value += 8  # Increment by 8 to account for preceeding requests to gee
